...
A panel at the bottom previews, for whatever item is highlighted (a service or a preset), the **full resolved list** of what it would start: primary services, their transitive dependencies, and any service-referencing `preCommands` — each with its mode.
